		...
		jb1.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try{
					...
					Connection conn = (Connection) DriverManager.getConnection("jdbc:mysql://localhost:3306/javaexternal","root", "mohit");
					if(conn!=null){
		...
		jb3.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try{
					...
					if(conn!=null){
						Statement stmt = (Statement) conn.createStatement();
						ResultSet rs = stmt.executeQuery("select * from Customer,Representative where Customer.repno=Representative.repno and climit>=15000;");
						String s="";
						ta.setText("");
						while(rs.next()){
							s += rs.getInt(1)+" ";
